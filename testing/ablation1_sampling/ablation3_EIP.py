...
from training.train_models.ablation3_ei.csmri import ModelSet as CSMRI
from training.train_models.ablation3_ei.ei import ModelSet as EI
from training.train_models.ablation3_ei.rotcsmri import ModelSet as ROTCSMRI

from training.train_models.ablation3_ei.csmri_ei import ModelSet as CSMRI_EI
from training.train_models.ablation3_ei.csmri_rotcsmri import ModelSet as CSMRI_ROTCSMRI
from training.train_models.ablation3_ei.ei_rotcsmri import ModelSet as EI_ROTCSMRI
...
def loop_test():
    ...
    cross_direction =['ixi_t1_periodic_slight_coronal', 'ixi_t1_periodic_moderate_coronal', 
                      'ixi_t1_periodic_slight_sagittal', 'ixi_t1_periodic_moderate_sagittal']

    one_motion = ['ixi_t1_periodic_slight_coronal']

    andi_heavy = ['andi_t1_periodic_moderate'] # for EIP quanlity results

    for motion in andi_heavy:
        test(motion)
...
def test(ourtta):
    ...
    # args.ablation_which = 'ablation1'
  

    # args.ablation_var = '20'
    modelset = CSMRI(args, test=True)
    model_csmri = modelset.model  
    model_csmri.eval()

    # args.ablation_var = '30'
    modelset = EI(args, test=True)
    model_ei = modelset.model  
    model_ei.eval()

    # args.ablation_var = '40'
    modelset = ROTCSMRI(args, test=True)
    model_rotcsmri = modelset.model  
    model_rotcsmri.eval()

    # args.ablation_var = '50'
    modelset = CSMRI_EI(args, test=True)
    model_csmri_ei = modelset.model  
    model_csmri_ei.eval()


    # args.ablation_var = '70'
    modelset = CSMRI_ROTCSMRI(args, test=True)
    model_csmri_rotcsmri = modelset.model  
    model_csmri_rotcsmri.eval()

    # args.ablation_var = '80'
    modelset = EI_ROTCSMRI(args, test=True)
    model_ei_rotcsmri = modelset.model  
    model_ei_rotcsmri.eval()
    ...
    df = {'csmri': None, 'ei': None, 'rotcsmri': None, 'csmri_ei': None, 'csmri_rotcsmri':None, 'ei_rotcsmri':None, 'our':None}
    ...
    with torch.no_grad():
        for x_iter in dataloader:
            ...
            pred_results = model_csmri(inputs)
            imgs['csmri'] = tensor2np(pred_results)

            pred_results = model_ei(inputs)
            imgs['ei'] = tensor2np(pred_results)

            pred_results = model_rotcsmri(inputs)
            imgs['rotcsmri'] = tensor2np(pred_results)

            pred_results = model_csmri_ei(inputs)
            imgs['csmri_ei'] = tensor2np(pred_results)

            pred_results = model_csmri_rotcsmri(inputs)
            imgs['csmri_rotcsmri'] = tensor2np(pred_results)

            pred_results = model_ei_rotcsmri(inputs)
            imgs['ei_rotcsmri'] = tensor2np(pred_results)

            pred_results = model_our(inputs)
            imgs['our'] = tensor2np(pred_results)


            get_metrics(args, imgs, df, shotname, NUM_SLICE)  # ? update psnrs and ssims , and get the current metrics - range -1~2000

            if ourtta == 'andi_t1_periodic_moderate':
