...
def test(ourtta):
    ...
    df = {'unet': None, 'restormer': None, 'stripformer': None, 'mstpp': None, 'our':None}
    ...
    with torch.no_grad():
        for x_iter in dataloader:
            ...
            pred_results = model_unet(inputs)
            imgs['unet'] = tensor2np(pred_results)

            pred_results = model_restormer(inputs)
            imgs['restormer'] = tensor2np(pred_results)

            pred_results = model_stripformer(inputs)
            imgs['stripformer'] = tensor2np(pred_results)

            pred_results = model_mstpp(inputs)
            imgs['mstpp'] = tensor2np(pred_results)
