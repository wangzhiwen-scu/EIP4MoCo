...
from training.train_models.csmri_newei_spatial import ModelSet as withoutBezier
from training.train_models.ACCS_csmri_and_ei_tvloss_prior_bezier import ModelSet as withBezier
...
def test(ourtta):
    ...
    # args.supervised_dataset_name = 'ixi_t1_periodic_slight'
    args.stage = '1'
    modelset = withoutBezier(args, test=True)
    model_wobezier = modelset.model
    model_wobezier.eval()

    modelset = withBezier(args, test=True)
    model_wbezier = modelset.model
    model_wbezier.eval()
    ...
    df = {'withoutbezier': None, 'withbezier': None}
    ...
    with torch.no_grad():
        for x_iter in dataloader:
            ...
            pred_results = model_wobezier(inputs)
            imgs['withoutbezier'] = tensor2np(pred_results)

            pred_results = model_wbezier(inputs)
            imgs['withbezier'] = tensor2np(pred_results)



            get_metrics(args, imgs, df, shotname, NUM_SLICE)  # ? update psnrs and ssims , and get the current metrics - range -1~2000

            if ourtta == 'ixi_t2_periodic_moderate':
