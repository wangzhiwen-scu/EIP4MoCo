...
from training.train_models.ablation6_vgg.csmri_newei_spatial import ModelSet as VGGModel

from training.train_models.csmri_newei_spatial import ModelSet as VGG5
...
def loop_test():
    ...
    bezier = ['ixi_t2_periodic_slight', 'ixi_t2_periodic_moderate', 'ixi_pd_periodic_slight', 'ixi_pd_periodic_moderate']

    quality = ['ixi_t2_periodic_moderate'] # for EIP quanlity results
...
def test(ourtta):
    ...
    # args.supervised_dataset_name = 'ixi_t1_periodic_slight'
    args.stage = '1'
    args.vggloss_weight = 0.0
    modelset = VGGModel(args, test=True)
    model_0 = modelset.model
    model_0.eval()

    args.vggloss_weight = 0.001
    modelset = VGGModel(args, test=True)
    model_3 = modelset.model
    model_3.eval()

    args.vggloss_weight = 0.0001
    modelset = VGGModel(args, test=True)
    model_4 = modelset.model
    model_4.eval()

    modelset = VGG5(args, test=True)
    ...
    df = {'vgg0': None, 'vgg3': None, 'vgg4': None, 'our': None}
    ...
    with torch.no_grad():
        for x_iter in dataloader:
            ...
            pred_results = model_0(inputs)
            imgs['vgg0'] = tensor2np(pred_results)

            pred_results = model_3(inputs)
            imgs['vgg3'] = tensor2np(pred_results)

            pred_results = model_4(inputs)
            imgs['vgg4'] = tensor2np(pred_results)
