...
def pandas_group_by(all_df, name, shot_shot_filename, keys_list):
    ...
    formatted_data, ssim_mean = process_data(df, keys_list)
    ...
    ssim_mean_df = ssim_mean.to_frame()
    ssim_mean_df.rename(columns={0 : 'SSIM'}, inplace=True)

    return formatted_data, ssim_mean_df
...
def process_data(df, keys_list):
    ...
    return combined_data, ssim_mean
...
def read_excel_and_plot(tabs, name, pdf_or_png, keys_list):
    ...
    formatted_data, ssim_mean = pandas_group_by(all_df, name, shot_shot_filename, keys_list)

    plt.figure(figsize=(9, 4))  # Specify the desired width and height

    # sns.lineplot(data=ssim_mean, x="Methods", y='SSIM', markers=True, dashes=False)

    ssim_results = ssim_mean.values


    plt.plot(keys_list, ssim_results, marker='o', alpha=0.4, linewidth=3)
    # plt.legend('')
    ...
    for i in range(len(keys_list)):
        plt.text(keys_list[i], ssim_results[i], str(ssim_results[i][0]), ha='center', va='bottom')

    # for i,j in zip(keys_list,ssim_results):
    #     plt.annotate(str(j),xy=(i,j))

    # plt.savefig('{}_{}_all.{}'.format(name, shot_shot_filename, pdf_or_png), dpi=300, bbox_inches='tight', transparent=True)
    plt.savefig('{}_{}_all.{}'.format(name, shot_shot_filename, pdf_or_png), dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    conditional_filename =  './results/ablation/ablation1_samplingratio/{}/'.format(filename) 
    mkdir(conditional_filename)

    tab1_a = {
                
                "Linear": './results/ablation/ablation1_samplingratio/ixi_t1_linear_moderate_Axial_None20230527-152626.xlsx',
                "Nonlinear": './results/ablation/ablation1_samplingratio/ixi_t1_nonlinear_moderate_Axial_None20230527-152939.xlsx',
                "Sudden": './results/ablation/ablation1_samplingratio/ixi_t1_sudden_moderate_Axial_None20230527-153435.xlsx',
                "Single-shot": './results/ablation/ablation1_samplingratio/ixi_t1_singleshot_moderate_Axial_None20230527-153943.xlsx',
                "RL": './results/ablation/ablation1_samplingratio/ixi_t1_periodic_slight_rl_Axial_None20230527-181035.xlsx',
                "Moderate": './results/ablation/ablation1_samplingratio/ixi_t1_periodic_moderate_Axial_None20230527-155023.xlsx',
                "Heavy": './results/ablation/ablation1_samplingratio/ixi_t1_periodic_heavy_Axial_None20230527-155530.xlsx',
    }

    keys_list = ['20', '30', '40', '50', '60', '70', '80', '90']
