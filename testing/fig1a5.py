...
def test():
    ...
    ourtta = 'ixi_t1_periodic_slight_rl'
    ...
    df = {'Corrupted': None, 'MARC': None, 'cycleGAN': None, 'BSA': None, 'Ours':None, 'Supervised':None}
    ...
    with torch.no_grad():
        for x_iter in dataloader:
            ...
            imgs['gt'] = tensor2np(torch_rotate(still, -90))

            imgs['Corrupted'] = tensor2np(torch_rotate(with_motion, -90))

            predicted_residuals = marc_model(inputs)
            recon_marc = inputs - predicted_residuals
            imgs['MARC'] = tensor2np(torch_rotate(recon_marc, -90))

            pred_results = cycle_model(inputs)
            imgs['cycleGAN'] = tensor2np(torch_rotate(pred_results, -90))

            recon_bsa = bsa_model(inputs)
            imgs['BSA'] = tensor2np(torch_rotate(recon_bsa, -90))

            # pred_results = cycle_model(inputs)
            # imgs['OursPrior'] = tensor2np(pred_results)

            recon_our = our_model(inputs)
            imgs['Ours'] = tensor2np(torch_rotate(recon_our, -90))

            recon_sup = supervised_model(torch_rotate(inputs, -90))
