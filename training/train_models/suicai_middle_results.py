...
# model setting
sys.path.append('.') # 
from testing.toolbox.test_toolbox import tensor2np
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(1, self.num_epochs+1):
            ...
            # self._run_epoch_rotei(epoch, dataloader_sup)
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            k_measure = out['masked_k']
            x_train = out['complex_abs']

            x_train_rec = self.model(x_train)

            loss = self.criterion(x_train_rec, y_train)  + self.criterion(self.fft(x_train_rec), self.fft(y_train))+  self.sobel_loss(x_train_rec, y_train) \
            ...
            if step == 1:
                ...
                self.plotter.k_space('k_measure', epoch, k_measure)
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            x_train, k_measure, img_measure = out_1st['complex_abs'], out_1st['masked_k'], out_1st['zero_filling']
            ...
            loss_eq =self.criterion(x_train_rec, rotate_x_train_rec_input) +  self.sobel_loss(x_train_rec, rotate_x_train_rec_input) \
            ...
            if step == 1:
                ...
                self.plotter.image('f1(y)', epoch, x_train_rec_mc)
                
                self.plotter.k_space('k_measure', epoch, k_measure)
                self.plotter.k_space('k_mc', epoch, k_mc)

                self.plotter.image('2nd_input', epoch, x_train)
                self.plotter.image('2nd_output', epoch, x_train_rec)
    ...
    def _run_epoch_rotcsmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            x_train_org = out['complex_abs']
            k_measure = out['masked_k']
            x_train_rec_1 = self.model(x_train_org)

            loss1 = self.criterion(x_train_rec_1, y_train) +  self.sobel_loss(x_train_rec_1, y_train) \
                      +  1e-3*self.vgg_loss(x_train_rec_1, y_train, feature_layers=[0, 1, 2, 3], style_layers=[])# 1

            rotate_ytrain = torch_rotate(y_train, 45)
            ...
            img_x = rotate_ytrain.detach().cpu().numpy()[0, 0, ...]
            img_out = x_train_rec.detach().cpu().numpy()[0, 0, ...]
            ...
            if step == 1:
                ...
                self.plotter.image('x_train_input', epoch, x_train_org)
                self.plotter.image('rec_1', epoch, x_train_rec_1)
                self.plotter.k_space('k_measure', epoch, k_measure)
    ...
    def _run_epoch_rotei(self, epoch, dataloader):
        # https://github.com/edongdongchen/REI/blob/b8b43fa35be09b16fa61873597ac8c6ffd244b31/rei/closure/ei_end2end.py#L7
        step = 0
        epoch_loss = 0
        self.metrics.ssims, self.metrics.psnrs = [], []
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            out_1st = self.sample_net(y_train)
            x_train, k_measure = out_1st['complex_abs'], out_1st['masked_k']
            x_train_rec_mc = self.model(x_train)

            y1_temp = self.sample_net(x_train_rec_mc)
            k_mc = y1_temp['masked_k']

            loss_mc = self.criterion(k_measure, k_mc) # because mc can 导致原来损坏的kdata，还是无法校正
